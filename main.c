...
void TIM16_IRQHandler(void)
{
	...
	uint8_t val = read_from_address(index);
	if(val==patterns[index]){
		LED_display(val);
	}
	else{
		LED_display(0b00000001);
	}

	//cycle to next binary array index
	if(index==5){
		index=0;
	}
	else{
...
// TODO: write a function 'display' value using LEDS
...
void LED_display(uint8_t val){
	...
	for(uint8_t j=0; j<8; j++){
		...
		if(j==0){GPIO_PIN_x = GPIO_PIN_0;}
		else if(j==1){GPIO_PIN_x = GPIO_PIN_1;}
		else if(j==2){GPIO_PIN_x = GPIO_PIN_2;}
		else if(j==3){GPIO_PIN_x = GPIO_PIN_3;}
		else if(j==4){GPIO_PIN_x = GPIO_PIN_4;}
		else if(j==5){GPIO_PIN_x = GPIO_PIN_5;}
		else if(j==6){GPIO_PIN_x = GPIO_PIN_6;}
		else if(j==7){GPIO_PIN_x = GPIO_PIN_7;}
